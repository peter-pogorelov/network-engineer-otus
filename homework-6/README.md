...
> Топология коммутаторов - симметрична, но BID у S1 меньше, чем у S2. Поэтому именно S1 у порт становится назначенным, а у S2 он блокируется.
> Далее, у S2 есть два пути до корня, прямой до S3 через f0/4 и транзитом через S1 через порт f0/2. Прямой путь лучше (19 < 38), поэтому он становится alternating и блокируется. 
> А путь через S1 становится рутовым путем.
